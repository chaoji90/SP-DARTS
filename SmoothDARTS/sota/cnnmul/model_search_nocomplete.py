...
class Network(nn.Module):
    ...
    def forward(self, input, temp=1,updateType='alpha'):
    ...
    def _loss(self, input, target, temp=1,updateType='alpha'):
        logits = self(input, temp,updateType=updateType)
    ...
    def genotype(self,temp=1):
        ...
        def _parse(weights, normal=True):
            ...
            for i in range(self._steps):
                ...
                #W = W_o[::-1]
                W = np.around(weights[start:end].copy(),decimals=3)
                ...
                while count<2:
                    ...
                    for j in reversed(range(i+2)):
                        for k in range(len(W[j])):
                            if PRIMITIVES[start+j][k]!='none':
                                if k_best is None or W[j][k] > W[j_best][k_best]:
                                    j_best=j
                                    k_best=k
                                    if W[j_best][k_best]==1.0:
                                        break
                        if W[j_best][k_best]==1.0:
                            break
                    if PRIMITIVES[start+j_best][k_best]!='skip_connect':
                        gene.append((PRIMITIVES[start+j_best][k_best], j_best))
                        count+=1
                    elif (PRIMITIVES[start+j_best][k_best]=='skip_connect' and skip_count<1) or len(W)==2:
                        gene.append((PRIMITIVES[start+j_best][k_best], j_best))
                        count+=1
                        skip_count+=1
                    if count>=2:
        ...
        gene_normal = _parse(F.softmax(self.alphas_normal/temp, dim=-1).data.cpu().numpy(), True)
        gene_reduce = _parse(F.softmax(self.alphas_reduce/temp, dim=-1).data.cpu().numpy(), False)
