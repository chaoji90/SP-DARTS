#python train_search.py --search_space=s1 --perturb_alpha=random
#python train_search.py --search_space=s1 --perturb_alpha=pgd_linf
...
from sota.cnnmul.model_search import Network
...
parser.add_argument('--arch_weight_decay', type=float, default=0.0, help='weight decay for arch encoding')
...
parser.add_argument('--C_start',type=float,default=0.8,help='start value of C')
...
def main():
    ...
    for epoch in range(args.epochs):
        if epoch>=warmup:
            temp=0.0003
            C+=(1-args.C_start+0.1)/(args.epochs-warmup)
        ...
        train_acc, train_obj = train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, 
                                         perturb_alpha, epsilon_alpha,temp,C)
        ...
        with torch.no_grad():
            ...
            logging.info('C = %f, softmax_temp= %f',C,temp)

        utils.save(model, os.path.join(args.save, 'weights.pt'))
    writer.close()


def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, perturb_alpha, epsilon_alpha,temp,C):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        architect.step(input, target, input_search, target_search, lr, optimizer, args.unrolled,softmax_list[softmax_rand[step]])
        optimizer.zero_grad()
        ...
        #logits = model(input, softmax_list[softmax_rand[step]],updateType='weight')
        logits = model(input, softmax_list[softmax_rand[step]])
