...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger, gradient_clip,warmup,epoch,softmax_temp,C):
  ...
  if epoch>=warmup: softmax_list=[softmax_temp*11,softmax_temp]
  else: softmax_list=[softmax_temp*10,softmax_temp*10]
  ...
  for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
    ...
    _, logits= network(base_inputs,softmax_list[softmax_rand[step]])
    ...
    _, logits= network(arch_inputs,softmax_list[softmax_rand[step]])
...
def main(xargs):
  ...
  for epoch in range(start_epoch, total_epoch):
    if epoch>=warmup: 
        if epoch>=45: softmax_temp=0.0006;C=C
        else: softmax_temp=0.0013;C+=0.9/(50-warmup)
    new_temp=softmax_temp

    w_scheduler.update(epoch, 0.0)
    need_time = 'Time Left: {:}'.format( convert_secs2time(epoch_time.val * (total_epoch-epoch), True) )
    epoch_str = '{:03d}-{:03d}'.format(epoch, total_epoch)
    logger.log('\n[Search the {:}-th epoch] {:}, LR={:}, temperature={:}, entropy={:}, C={:}'.format(epoch_str, need_time, min(w_scheduler.get_lr()),new_temp,entropy_n,C))

    search_w_loss, search_w_top1, search_w_top5 = search_func(search_loader, network, criterion, w_scheduler, w_optimizer, a_optimizer, epoch_str, xargs.print_freq, logger, xargs.gradient_clip,warmup,epoch,new_temp,C)
    ...
    with torch.no_grad():
      ...
      logger.log('{:}'.format(search_model.show_alphas(new_temp*10)))
